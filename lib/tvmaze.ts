...
// stored there, including Supabase's own session token). This cache grows
// unbounded — every show/episode list ever viewed, kept around past its TTL
// as a stale fallback — so on web it eventually filled that shared quota
// entirely, to the point where even Supabase's small auth-token write
// started failing with QuotaExceededError on sign-in. Its own IndexedDB
// database (hundreds of MB, isolated from localStorage) fixes that.
const storage = createAsyncStorage("tvmaze_cache");
